...
留到 Phase 5 實際部署時一併測試。`pwa-next/` 的閱讀器 UI 尚未跟 `pwa/` 逐項比對過完整
功能對等，離「可以切換上線」還有距離，見下方 Phase 5。**
...
- [x] 情境：清除本機瀏覽器資料後重新登入，確認進度/書籤/註記皆能還原（2026-08-01 使用者驗證通過）
- [x] 情境：書本檔案遺失後重新匯入同一份 epub，確認能自動接回舊紀錄（2026-08-01 使用者驗證通過）
- [x] 情境：匯入內容不同、但檔名相同的另一本書，確認不會被誤判成同一本書接錯資料（2026-08-01 使用者驗證通過）
- [x] 情境：離線時（無網路）本機閱讀/畫記是否仍可正常運作，恢復連線後能否正確同步（2026-08-01 使用者驗證通過）
