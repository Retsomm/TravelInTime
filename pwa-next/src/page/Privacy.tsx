...
const SECTIONS: { title: string; paragraphs: string[] }[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '本應用所有書籍與使用者建立的內容（進度、書籤、註記等）僅保存在你的裝置上，不會上傳到任何伺服器或雲端資料庫，也沒有帳號或登入機制。',
      '第三方連線（會與第三方伺服器通訊，但不含你的書籍/註記內容）：Google Fonts（載入遠端字型）；Electron 版自動更新機制檢查更新時會向更新服務發送請求。',
  ...
  },
  {
    title: '資料儲存與保存地點',
    paragraphs: [
      '所有資料僅儲存在使用者裝置上（例如 IndexedDB、localStorage、SQLite、App 沙盒檔案系統），完全不會上傳至任何伺服器。',
      '第三方請求（如 Google Fonts、更新伺服器）會在對應時機發出 HTTPS 請求，可能使第三方接收到裝置的 IP 位址與相關連線資訊。',
    ],
  },
  {
    title: '資料分享與第三方',
    paragraphs: [
      '我們不會將使用者的書籍或註記分享或出售給第三方，因為這些內容從未離開你的裝置。',
  ...
  },
  {
    title: '如何刪除資料',
    paragraphs: [
      'App（書庫）：長按或選擇該書 → 點「刪除」，即可移除該書的本機註記/進度。',
