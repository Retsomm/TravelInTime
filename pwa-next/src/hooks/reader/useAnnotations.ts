...
// 這裡改成每個 Reader session 自己持有一份、依 bookId 綁定的 React state，持久化跟遠端
// 同步直接內建在每個 mutation 函式裡（persist()），不再需要外部用 .subscribe() 側效應去監聽
// 「任何一次 state 變動」再回頭存檔——原本那個 subscribe 模式必須小心處理呼叫順序（見舊版
// useReaderEngine.ts 的「先 unsub 再 clearAll，避免 clear 覆蓋 localStorage」註解），
// 這裡直接消除了整類排序陷阱。
//
...
export const useAnnotations = (bookId: string) => {
  ...
  const loadForBook = useCallback((loaded: Annotation[]) => setAnnotations(loaded), [])

  const addAnnotation = useCallback((a: Omit<Annotation, 'id' | 'createdAt' | 'updatedAt'>): string => {
    const id = crypto.randomUUID()
    const now = Date.now()
    persist([...annotations, { ...a, id, createdAt: now, updatedAt: now }])
    return id
  }, [annotations, persist])
  ...
  const updateNote = useCallback((id: string, note: string) => {
    persist(annotations.map((a) => (a.id === id ? { ...a, note: note.trim() || undefined, updatedAt: Date.now() } : a)))
  }, [annotations, persist])
  ...
  return { annotations, loadForBook, addAnnotation, removeAnnotation, updateColor, updateNote, clearAll }
}
