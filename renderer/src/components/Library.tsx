...
const Library = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('[handleFileChange] 觸發，files:', files?.length ?? 0)
    if (!files || files.length === 0) {
      console.warn('[handleFileChange] 未選擇任何檔案')
      return
    }
    const fileArray = Array.from(files)  // FileList 是 live 物件，必須先轉為靜態陣列
    e.target.value = ''
    setLoading(true)
    try {
  ...
  if (records.length === 0) {
    return (
      <div className="drag-region flex flex-col min-h-screen">
        <div className="flex items-center justify-between pl-20 pr-4 py-3 shrink-0">
        ...
        <button
          className="no-drag px-8 py-4 bg-stone-800 dark:bg-stone-100 text-white dark:text-stone-900 rounded-2xl text-lg font-medium hover:bg-stone-700 dark:hover:bg-stone-200 transition disabled:opacity-50"
