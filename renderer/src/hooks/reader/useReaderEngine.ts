...
export const useReaderEngine = (params: {
  ...
  const currentSpineIdxRef = useRef<number | null>(null) // 追蹤當前合法的 spine 索引
  ...
  useEffect(() => {
    ...
    onUpdateProgress?.(clampProgressRatio(rawRatio, ttsActiveRef.current))
    ...
    fetch(bookPath)
      ...
      .then((buffer) => {
        ...
        rendition.hooks.content.register((view: unknown) => {
          ...
          doc.addEventListener('touchstart', () => { noteUserInteraction(); setPopup(null); setEditPopup(null) })
        ...
        rendition.on('relocated', (loc: unknown) => {
          ...
          // 補救：epub.js 的 annotations.inject(view) 在跳頁後偶爾未能重新繪製既有註記的 SVG
          ...
          if (ttsAutoFollowBusyRef.current && pendingAutoFollow) {
            ...
            ttsAutoFollowUnlockTimerRef.current = setTimeout(() => {
              ...
              if (DEBUG_TTS_FOLLOW) console.log('[TTS:follow] relocated 後已解鎖自動翻頁', { sequence })
            }, 450)
          }
  ...
  const followTTSRange = (range: Range | null, doc: Document, absoluteOffset: number, allowAutoFollow: boolean, source: TTSProgressSource) => {
    ...
    if (DEBUG_TTS_FOLLOW) {
      ...
      if (shouldAdvance || source === 'boundary' || now - ttsLastFollowDebugAtRef.current >= 1000) {
        ...
        console.log('[TTS:follow] check', {
          source, displayed, absoluteOffset,
          visibleSpineIdx: loc?.start?.index,
          readingSpineIdx: ttsVisibleSpineIndexRef.current,
          pageStartOffset, pageEndOffset, pageTurnOffset, distanceToPageEnd,
          chapterTextLength: textLength, estimatedPage, continuousPage: Number(continuousPage.toFixed(2)),
          currentPage, totalPages,
          pageEntryGuard, beforeCurrentPageGuard, insideNewPageGuard,
          measuredPageEndReached, progressShouldAdvance, shouldAdvance,
        })
      }
    }
    ...
    if (DEBUG_TTS_FOLLOW) console.log('[TTS:follow] next() 延後自動翻到下一頁', {
      displayed, continuousPage: Number(continuousPage.toFixed(2)), absoluteOffset,
      pageStartOffset, pageEndOffset, pageTurnOffset, distanceToPageEnd,
  ...
  const speakCurrentPage = () => {
    ...
    // 取得 spine 索引：優先用 location，備選用已追蹤的 currentSpineIdxRef
